document.addEventListener('DOMContentLoaded', function () {
  ...
  // Obtém os dados do Local Storage ou cria um array vazio caso não haja dados
  let favUsers = JSON.parse(localStorage.getItem('favUsers')) || [];

  // Função para renderizar a tabela com os dados dos usuários favoritados
  function renderTable() {
    tableBody.innerHTML = '';

    if (favUsers.length === 0) {
      // Caso não haja usuários favoritados, exibe uma linha na tabela informando
      tableBody.innerHTML = `
    ...
    `;
      ...
      favUsers.forEach(user => {
        ...
        row.innerHTML = `
          ...
          <td>
            <button class="remove" data-login="${user.login}">Remover</button>
          </td>
        `;
  ...
  // Função para adicionar um usuário na lista de favoritos
  function addFavUser(user) {
    const isUserNotFound = user.message && user.message.toLowerCase().includes('not found');
    const isUserAlreadyFavorited = favUsers.find(favUser => favUser.login === user.login);
  
    if (isUserNotFound || isUserAlreadyFavorited) {
      // Se o usuário não foi encontrado ou já está na lista de favoritos, exibe o input com borda vermelha
      input.classList.add('invalid');
      return;
    }
  
    // Remove a classe 'invalid' do input caso o usuário exista
  ...
  // Função para remover um usuário da lista de favoritos
  function removeFavUser(login) {
    favUsers = favUsers.filter(user => user.login !== login);
    localStorage.setItem('favUsers', JSON.stringify(favUsers));
    renderTable();
  }
  ...
  // Evento para remover um usuário da lista de favoritos
  ...
  // Inicializa a tabela com os dados dos usuários favoritados
  renderTable();
});
